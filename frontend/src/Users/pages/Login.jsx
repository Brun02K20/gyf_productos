...
const Login = () => {    
    ...
    const { onSubmit, errorMessage, setErrorMessage } = useLoginForm("login");
      ...
      <div className="w-full max-w-md">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-indigo-200 p-8 space-y-6">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Bienvenido</h1>
            <p className="text-indigo-600/70 text-sm mt-2">Inicia sesión en tu cuenta</p>
          </div>
          ...
          <form id="login-htmlForm" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-semibold py-2.5 rounded-lg transition-all duration-300 shadow-lg shadow-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/60 hover:scale-105 cursor-pointer"
            >
              Iniciar sesión
            </button>
          </form>
          ...
          <p className="text-center text-sm text-indigo-600/70">
            ¿No tienes cuenta?{" "}
            <a href="/register" className="font-semibold text-indigo-600 hover:text-purple-600 transition-colors">
              Regístrate aquí
            </a>
          </p>
        </div>
