...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loggedUserJSON = window.localStorage.getItem("GYFProductos");
    if (loggedUserJSON) {
  ...
  const login = (userData) => {
    window.localStorage.setItem("GYFProductos", JSON.stringify(userData)); // Guardamos los datos en localStorage
    setUser(userData);
